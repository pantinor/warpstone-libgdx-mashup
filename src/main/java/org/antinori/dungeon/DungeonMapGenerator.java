...
public class DungeonMapGenerator {
	...
	public DungeonMapGenerator(int width, int height) {
		...
		heightValues = new double[width][height];
	 ...
	 * 
	 * @param x
	 *            The x coordinate at which to set the tile
	 * @param y
	 *            The y coordinate at which to set the tile
	 * @param tile
	 *            The tile to set or 0 to clear
	 */
	public void setTile(int x, int y, int tile) {
		data[x][y] = tile;
		if (tile == 0) heightValues[x][y] = 0;
	}

	public void setTileHeight(int x, int y, int height) {
		heightValues[x][y] = height;
	}
	 ...
	 * 
	 * @param x
	 *            The x coordinate at which to get the tile
	 * @param y
	 *            The y coordinate at which to get the tile
	 ...
	 * @param minRooms
	 *            The minimum number of rooms to accept in a generation
	 * @param mergeIterations
	 *            How many times we'll try to merge rooms together
	 */
	public void generate(long seed, int minRooms, int mergeIterations, int corridorCount) {
		...
		while (totalRooms < minRooms) {
			...
			// rooms
			// linked to it, and rooms linked to those and so on, to be filled
			// with the
			// same ID. If we find any rooms without an "island ID" then they
			// are completely
			// separated.
			int islandID = 1;
			int bestCount = 0;
