...
public class LightManager implements Serializable {
	...
	public void setAmbient(Color color, Vector3 dir) {
	...
	public Vector3 calculateLightAtPoint(Vector3 position, Vector3 normal, boolean bakeStatics) {
		Vector3 h_ambient = getAmbientRGB().mul(0.5f);
	...
	private Vector3 calculateLight(Vector3 l_vector, Vector3 l_colour, float l_attenuation, float l_power, Vector3 n_dir) {
		...
		// Intensity of the diffuse light. Saturate to keep within the 0-1
		// range.
		float NdotL = n_dir.dot(l_dir);
		float intensity = MathUtils.clamp(NdotL, 0.0f, 1.0f); // Math.max(0.0f,
																// n_dir.dot(l_dir)
		...
		// Calculate the diffuse light factoring in light color, power and the
		// attenuation
	...
	public Vector3 getAmbientRGB() {
		return new Vector3(ambientLight.r, ambientLight.g, ambientLight.b);
	}
}